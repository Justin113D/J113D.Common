...
namespace J113D.Common
{
	...
	public sealed class DistinctMap<T> : DistinctMap
	{
		/// <summary>
		/// Collection of unique values
		/// </summary>
		public new IList<T> Values { get; }
		...
		public DistinctMap(IList<T> values, int[]? map) : base(new ReadOnlyCollection<T>(values), map)
		{
			Values = values;
		}
		...
		/// <returns></returns>
		public new T GetValue(int absoluteIndex)
		{
	...
	/// <summary>
	/// Generic base class for <see cref="DistinctMap{T}"/>
	/// </summary>
	public class DistinctMap
	{
		...
		internal DistinctMap(IList values, int[]? map)
		{
			Values = values;
			Map = map;
		}
